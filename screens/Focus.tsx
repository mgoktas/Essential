...
import { DefaultPickerSessionNumber, getRemaining, } from '../components/Functions/Functions';
import { examples, getData, getDataNumber, getDataString } from '../components/Storage/Data';
...
const Focus = ({navigation}) => {
    ...
    if(isTaskModeOn && !isLoading){
        ...
        return <SafeAreaView style={[styles.pageFocus]}>
            <CaretIcon mode={2} icon={'menu'} onPress={() => {navigation.navigate('Settings', {isLogged: isLogged})}}/>
            <CaretIcon2 isDoroModeOn={isDoroModeOn} onPress={() => {openTaskMode(); if(isTaskModeOn){resetAll()}}}/>
            <ImageBackground style={styles.pageFocusImage} source={examples[getDataNumber('defaultDoroInt')-1].source}>
